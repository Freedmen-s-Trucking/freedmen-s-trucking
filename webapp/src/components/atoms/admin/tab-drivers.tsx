...
const DriverManagement: React.FC = () => {
  ...
  const { mutate: updateDriverStatus } = useMutation({
    mutationFn: async ({
      ...
      if (!currentDriver?.data.user?.uid || currentDriver?.path !== path)
        return;
      return _updateDriver(currentDriver.data.user.uid, {
        verificationStatus: status,
      });
    },
    onSuccess(_, { path, status }) {
            ...
            <div>
              <div className="mb-6 flex flex-col items-start gap-6 md:flex-row">
                ...
                <div className="w-full md:w-2/3">
                  <Tabs theme={tabTheme} style="underline">
                    <Tabs.Item
                      title={
                        <div className="flex items-center gap-2">
                          <HiIdentification />
