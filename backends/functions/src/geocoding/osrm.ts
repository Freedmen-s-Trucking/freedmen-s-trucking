...
const routingUrl = (startingPoint: GeoPoint, endPoint: GeoPoint) => {
  return `http://router.project-osrm.org/route/v1/driving/${endPoint.longitude},${endPoint.latitude};${startingPoint.longitude},${startingPoint.latitude}`;
};
...
export class OSRMGeoRoutingService implements GeoRoutingInterface {
  ...
  async getDistanceInKilometer(
    ...
    const routingResponse =
      response.data as null | Array<GetDistanceInKilometerResponse>;
