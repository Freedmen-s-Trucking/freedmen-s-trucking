...
export async function postRequest({
  ...
  try {
    ...
    console.error(" [Axios] failed to send post request error: " + error + "\n route : " + route);
    return undefined;
  }
}
...
export async function getRequest({
  ...
  try {
    ...
    console.log(
      `Request completed successfully with status ${response.status} and data : ${JSON.stringify(response.data)}`,
    );
    ...
    console.error(" [Axios] failed to send get request error: " + "\n route : " + route);
