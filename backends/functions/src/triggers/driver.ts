...
import {DocumentReference, FieldValue, getFirestore, PartialWithFieldValue} from "firebase-admin/firestore";
...
const updatePlatformOverviewOnDriverVerificationStatusChange = async (
  ...
  const platformOverviewDoc = getFirestore().doc(
    LATEST_PLATFORM_OVERVIEW_PATH,
  ) as DocumentReference<PlatformOverviewEntity>;
  const prevPlatformOverview = (await platformOverviewDoc.get()).data() as PlatformOverviewEntity;
  // TODO: Handle driver license verification status.
  if (aVStatus !== bVStatus) {
    const update = {} as PartialWithFieldValue<PlatformOverviewEntity>;
    switch (aVStatus) {
      case "pending":
        update.totalPendingVerificationDrivers = (prevPlatformOverview.totalPendingVerificationDrivers || 0) + 1;
        break;
      case "failed":
        update.totalFailedVerificationDrivers = (prevPlatformOverview.totalFailedVerificationDrivers || 0) + 1;
        break;
      case "verified":
        update.totalVerifiedDrivers = (prevPlatformOverview.totalVerifiedDrivers || 0) + 1;
        break;
    }
    switch (bVStatus) {
      case "pending":
    ...
    if (Object.keys(update).length > 0) {
      ...
      await platformOverviewDoc.set(update, {
        merge: true,
      });
    }
  }
