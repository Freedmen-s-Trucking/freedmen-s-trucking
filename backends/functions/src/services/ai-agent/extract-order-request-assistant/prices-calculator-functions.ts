...
export function computeTheMinimumRequiredVehiclesAndFees(
  ...
  while (currentPackage) {
    ...
    const totalPkgVolumeToUse = remainingVolume + pkgVolume * currentPackage.quantity;
    ...
    if (remainingVehicleVolume >= 0) {
      ...
      remainingWeight = currentPackage.estimatedWeightInLbsPerUnit * currentPackage.quantity;
      currentPackageId++;
...
function getFeesOfVehicle(
  ...
  const mileageCost = vehicleInfo.pricePerMile * distanceInMiles * vehicle.quantity;

  const priorityAddonCost = deliveryPriorityPricesAddons[priority] * distanceInMiles * vehicle.quantity;
  ...
  const totalCost =
    (basePlatformFeeInUSD + mileageCost + priorityAddonCost + weightCost + volumeCost) *
...
function findVehicleIdBasedOnVolume(cuFeet: number): {
  ...
  return {
    ...
    remainingVolume: vehiclesInfoList[vehiclesInfoList.length - 1].avgCapacityInCubicFeet - cuFeet,
  };
}
...
export async function computeDeliveryEstimation(params: Partial<ComputeDeliveryEstimation>) {
  const {pickupLocation, deliveryLocation, priority, products} = params;
  const gdr = await getDistance(pickupLocation!, deliveryLocation!);
  const distanceData = {
    distanceMeasurement: DistanceMeasurement.OSRM_FASTEST_ROUTE,
    distanceInMiles: metersToMiles(gdr.routes[0].distance),
    durationInSeconds: gdr.routes[0].duration,
  };
